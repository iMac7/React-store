      ...
      {({loading, data})=> {
        ...
        return (
            data.categories.map(category =>
              this.props.selected === category.name &&
                ...
                <div className={styles.gallery}>
                  {
                    category.products.map(product => (
                      <Link to={`/${product.id}/description`} className={styles.link} key={product.id}>
...

                        {
                          this.props.cart.map(item => {
                            if(item.id === product.id) return(
                              <svg key='svg' width="74" height="74" viewBox="0 0 74 74" fill="none" xmlns="http://www.w3.org/2000/svg">
