...
import { get_products, fetch_description } from '../Graphql/Queries';
...
export default class Categories extends Component {
  ...
  render() {
    ...
    return (
      ...
      variables={{title: this.props.selected}}>
      {({loading, data})=> {
        if(loading) return <h2>loading...</h2>

        return (
            this.props.match.params.home === data.category.name &&
              <div className={styles.categories} key={data.category.name}>
                ...
                <div className={styles.gallery}>
                  {
                    data.category.products.map(product => (
                      ...
                      <Link to={`${this.props.selected}/${product.id}/description`} className={styles.link} key={product.id}>
                        <div className={styles.card}>
                          <img src={product.gallery[0]} alt="" />
                          <div className={styles.name}>{product.brand}&nbsp;{product.name}</div>
                          <h4 className={styles.price}>
                            {
                            product.prices.map(price => price.currency.symbol === this.props.currency && (
                              <div key={price.currency.symbol}>{price.currency.symbol}&nbsp;{price.amount}</div>
                            ))
                            }
                          </h4>
                        </div>
                        ...
                        <svg className={styles.svg} 
                          onClick={(e) => {
                            ...
                            .then((res) => res.json())
                            .then((result) => {
                              let fetched = result.data.product
                              
                              let selectedAttributes = {}
                              //pick first item of each attribute
                              fetched.attributes?.map(attribute => { 
                                return selectedAttributes[attribute.name] = attribute.items[0].displayValue
                              })
                              ...
                              this.props.addtocart({
                                ...
                                allAttributes: fetched.attributes,
                         ...
                         width="74" height="74" viewBox="0 0 74 74" fill="none" xmlns="http://www.w3.org/2000/svg">
